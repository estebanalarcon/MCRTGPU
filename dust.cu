...
DustTemperature* allocateMemoryToDustTemperature(int numSpec, Grid* grid){
...
void deallocateCumulEner(DustTemperature* dustTemperature, int numSpec, int ny, int nz){
...
void deallocateDustTemperature(DustTemperature* dustTemperature, int numSpec, int ny, int nz){
...
void inicializeDustTemperature(DustTemperature* dustTemperature, DustDensity* dustDensity){
...
DustDensity* allocateMemoryToDustDensity(int iFormat, int numCells, int numSpec, Grid* grid){
  int nx = grid->nCoord[0];
  int ny = grid->nCoord[1];
  int nz = grid->nCoord[2];
...
void deallocateDensities(DustDensity* dustDensity){
  for (int iSpec=0 ; iSpec<dustDensity->numSpec ; iSpec++){
...
DustDensity* dustDensityTransferToDevice(DustDensity* h_dustDensity){
...
void writeDustTemperature(DustTemperature* h_dustTemperature, DustTemperature* d_dustTemperature, DustDensity* h_dustDensity){
...
DustTemperature* setUpDustTemperature(DustDensity* dustDensity, Grid* grid){
