...
#include "frequencies.cu"
#include "grid.cu"
#include "stars.cu"
#include "dust.cu"
#include "opacities.cu"
#include "emissivity.cu"
#include "structs.cuh"
...
__device__ void findNewFrequencyInu(Photon* photon, curandState* state, double* specCum, FrequenciesData* freqData){
  ...
  //printf("random=%lf\n",rn );
  //int rayInu = 0;
  //huntCPU(specCum, freqData->numFrequencies+1, rn, &rayInu);
  huntCPU(specCum, freqData->numFrequencies+1, rn, &(photon->iFrequency));
  //printf("rayInu = %d\n",rayInu);
  //return rayInu;
}
...
__device__ void findStar(Photon* photon, Stars* d_stars, curandState* state){
  ...
  if (d_stars->numStars > 1){
    ...
    //huntCPU(d_stars->luminositiesCum, d_stars->numStars+1, rn, istar);
  }
}
...
void deallocatePhotons(Photon* photons, int numPhotons){
  for (int i=0 ; i< numPhotons ; i++){
